...
type TabId = 'dashboard' | 'weddings' | 'users' | 'audit'
...
function DashboardTab({ fetchWithAuth }: { fetchWithAuth: (url: string, init?: RequestInit) => Promise<Response | null> }) {
  ...
  // MRR estimated — sum of priceUsd of each recent wedding's plan
  // (a rough proxy until the Subscription model is wired up)
  const mrrEstimate = (data.recentWeddings || []).reduce(
    (sum, w) => sum + (PLAN_METADATA[w.plan]?.priceUsd || 0),
    0
  )
  ...
  const kpiCards = [
    {
      ...
      subtitle: `${data.weddings.byStatus?.PUBLISHED ?? 0} publiés`,
      icon: Heart,
    ...
    },
    {
      ...
      subtitle: `${data.users.platformAdmins ?? 0} admins plateforme`,
      icon: UsersIcon,
    ...
    },
    {
      ...
      subtitle: `${data.guests.last7days ?? 0} derniers 7j`,
      icon: UsersIcon,
