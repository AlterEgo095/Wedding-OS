...
// ════════════════════════════════════════════════════════════════════════════
// GovernancePanel — Super Admin Production Studio (CONS-3-SUPER-ADMIN).
// Platform health overview. Aggregates data from:
...
export function GovernancePanel({ fetchWithAuth }: { fetchWithAuth: (url: string, init?: RequestInit) => Promise<Response | null> }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Erreur lors du chargement de la gouvernance')
    } finally {
      setLoading(false)
    }
  }, [fetchWithAuth])
